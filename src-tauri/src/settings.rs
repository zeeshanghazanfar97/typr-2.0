...
const POLISH_PROMPT: &str = r#"You are "Polish," a high-quality text refinement transform inside a dictation app.
...
impl Settings {
    ...
    fn normalized_system_prompt(id: &str, system_prompt: &str, fallback: &TextTransform) -> String {
        ...
        if system_prompt.is_empty() {
            ...
            && (system_prompt == LEGACY_POLISH_PROMPT || system_prompt == PREVIOUS_POLISH_PROMPT)
        {
...
mod tests {
    ...
    #[test]
    fn test_normalize_transforms_migrates_previous_polish_default_prompt() {
        let mut settings = Settings::default();
        settings.transforms = vec![TextTransform {
            id: "polish".to_string(),
            name: "Polish".to_string(),
            icon: "sparkles".to_string(),
            system_prompt: PREVIOUS_POLISH_PROMPT.to_string(),
        }];
